import { getRoundWin, mapSteamIDToPlayer, parseTeam, getHalfFromRound, didTeamWinThatRound, parseGrenades } from './utils.js';
class CSGOGSI {
    constructor() {
        this.eventNames = () => {
        ...
        this.digest = (raw) => {
            ...
            if (raw.round && raw.map && raw.map.round_wins) {
                ...
                for (let i = 1; i <= currentRound; i++) {
                    const result = getRoundWin(currentRound, { ct: teamCT, t: teamT }, raw.map.round_wins, i, this.regulationMR, this.overtimeMR);
                    if (!result)
                        continue;
                    rounds.push(result);
                }
            ...
            if ((raw.map.round === 0 && raw.phase_countdowns.phase === 'freezetime') ||
                ...
                bomb: bomb
                    ? {
                        ...
                        site: bomb.state === 'planted' ||
                            ...
                            bomb.state === 'defusing' ||
                            bomb.state === 'planting'
                            ? CSGOGSI.findSite(raw.map.name, bomb.position.split(', ').map(n => parseFloat(n)))
                            : null
                    }
                    : null,
        ...
        this.digestMIRV = (raw, eventType = 'player_death') => {
