...
export const getHalfFromRound = (round, regulationMR, mr) => {
...
export const didTeamWinThatRound = (team, round, wonBy, currentRound, regulationMR, mr) => {
...
export const parseGrenades = (grenades) => {
    if (!grenades)
        return [];
    return Object.entries(grenades).map(([id, grenade]) => parseGrenade(grenade, id));
};
export const getRoundWin = (mapRound, teams, roundWins, round, regulationMR, overtimeMR) => {
