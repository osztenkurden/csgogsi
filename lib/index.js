...
class CSGOGSI {
    ...
    digest(raw) {
        ...
        const bomb = raw.bomb;
        const teamCT = utils_1.parseTeam(raw.map.team_ct, isCTLeft ? 'left' : 'right', 'CT', isCTLeft ? this.teams.left : this.teams.right);
        const teamT = utils_1.parseTeam(raw.map.team_t, isCTLeft ? 'right' : 'left', 'T', isCTLeft ? this.teams.right : this.teams.left);
        const playerMapper = utils_1.mapSteamIDToPlayer(raw.allplayers, { CT: teamCT, T: teamT }, this.players);
