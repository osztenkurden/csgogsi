...
exports.parseTeam = exports.mapSteamIDToPlayer = exports.CSGOGSI = void 0;
...
class CSGOGSI {
    ...
    digest(raw) {
        ...
        const rounds = [];
        if (raw.round && ((_a = raw.map) === null || _a === void 0 ? void 0 : _a.round_wins)) {
            let currentRound = raw.map.round + 1;
            if (raw.round && raw.round.phase === 'over') {
                currentRound = raw.map.round;
            }
            for (let i = 1; i < currentRound; i++) {
                const result = utils_js_1.getRoundWin(currentRound, { ct: teamCT, t: teamT }, raw.map.round_wins, i, this.MR);
                if (!result)
                    continue;
                rounds.push(result);
            }
